...
void ArbolBA<T,G>::MezclarEnDos(Nodo *&subRaiz, Nodo *hjoI, Nodo *hjoM, Nodo *hjoD, unsigned i)
{
    ...
    for(unsigned j=i+1;j<subRaiz->nE;++j)
        subRaiz->elemento[j]=subRaiz->elemento[j+1];
    subRaiz->elemento[subRaiz->nE-1]=NULL;
    subRaiz->elemento[subRaiz->nE]=NULL;

    for(unsigned j=i+2;j<=subRaiz->nE;++j)
        subRaiz->hijo[j]=subRaiz->hijo[j+1];
    subRaiz->hijo[subRaiz->nE]=NULL;
    subRaiz->hijo[subRaiz->nE+1]=NULL;
...
void ArbolBA<T,G>::CompartirD(Nodo *&subRaiz, Nodo *hjoI, Nodo *hjoD, unsigned i)
{
    ...
    hjoI->elemento[hjoI->nE - 1]=NULL;
    for(unsigned j = hjoD->nE; j > 0; --j) hjoD->elemento[j] = hjoD->elemento[j-1];
    hjoD->elemento[0] = aux;

    for(unsigned j = hjoD->nE+1; j > 0; --j) hjoD->hijo[j] = hjoD->hijo[j-1];
